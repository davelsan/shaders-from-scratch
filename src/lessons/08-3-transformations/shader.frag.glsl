...
void main() {
  ...
  // - Translation sign is inverted in SDF manipulation
  vec2 circlePos = pixelCoords;
  circlePos.x += 100.0 * sin(uTime);
  float circleDist = sdfCircle(circlePos, 100.0); // centered

  // Square: distance of the pixel to a box shape
  // - Normally rotate first then translate, but in SDF manipulations it is reversed
  vec2 boxPos = pixelCoords;
  boxPos.y += edge.y * 0.6;
  boxPos *= rotate2d(uTime * 0.25);
  float boxDist = sdfBox(boxPos, vec2(100.0, 50.0)); // translated
  ...
  starPos.y -= edge.y * 0.6;
  starPos *= rotate2d(uTime * 0.25);
  starPos += 50.0 * sin(uTime);
  float heartDist = sdfStar(starPos, 50.0); // translated
  ...
  color = mix(red, color, step(0.0, circleDist));

  // red pixels inside the box
  color = mix(red, color, step(0.0, boxDist));

  // red pixels inside the star
  color = mix(red, color, step(0.0, heartDist));
