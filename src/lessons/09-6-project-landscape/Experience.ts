...
import { backgroundBlur, foregroundBlur } from './State';

type Uniforms = {
  uResolution: Vector2;
  uTime: number;
  // Debug
  uBackgroundBlur: number;
  uForegroundBlur: number;
};
...
export class Experience extends WebGLView {
  ...
  private setupMaterial = ({ viewport }: ThreeState) => {
    this.material = new ShaderMaterial({
      ...
      uniforms: {
        ...
        uTime: 0,
        //
        uBackgroundBlur: backgroundBlur.get(),
        uForegroundBlur: foregroundBlur.get(),
      },
    });
  };
  ...
  private setupSubscriptions = ({ viewport }: ThreeState) => {
    ...
    backgroundBlur.sub((value) => {
      this.material.uBackgroundBlur = value;
    });

    foregroundBlur.sub((value) => {
      this.material.uForegroundBlur = value;
    });
